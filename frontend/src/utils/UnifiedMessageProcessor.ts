...
export interface ProcessingContext {
  ...
  // Current message ID ref for tracking streaming messages (avoids stale closure)
  currentMessageIdRef?: MutableRefObject<string | null>;

  // Accumulated content ref for tracking streamed text (avoids stale closure)
  accumulatedContentRef?: MutableRefObject<string>;
}
...
export class UnifiedMessageProcessor {
  ...
  private handleAssistantText(
    ...
    // Use message ID from backend to track the current streaming message
    // This avoids stale closure issues with React state
    const currentMessageId = context.currentMessageIdRef?.current;

    // Determine if this is a new message:
    // - If we have a messageId and it differs from current, it's new
    // - If we have no messageId but also no currentMessageId, it's new
    // - If we have no messageId but have currentMessageId, continue existing
    const isNewMessage = messageId
      ? currentMessageId !== messageId
      : !currentMessageId;

    if (isNewMessage) {
      // This is a new message - create it and track the ID
      if (context.currentMessageIdRef && messageId) {
        context.currentMessageIdRef.current = messageId;
      } else if (context.currentMessageIdRef && !messageId) {
        // Generate a temporary ID for tracking when backend doesn't provide one
        context.currentMessageIdRef.current = `temp-${Date.now()}`;
      }

      // Reset accumulated content for the new message
      const initialContent = contentItem.text || "";
      if (context.accumulatedContentRef) {
        context.accumulatedContentRef.current = initialContent;
      }
      ...
      context.setCurrentAssistantMessage?.(newMessage);
      context.addMessage(newMessage);
      // Also call updateLastMessage for consistency with streaming UI updates
      context.updateLastMessage?.(newMessage.content);
    } else {
      // Same message - append to accumulated content via ref (avoids stale closure)
      const prevContent = context.accumulatedContentRef?.current ?? "";
      const updatedContent = prevContent + (contentItem.text || "");

      // Update the ref immediately so the next chunk sees the latest content
      if (context.accumulatedContentRef) {
        context.accumulatedContentRef.current = updatedContent;
      }
      ...
      const updatedMessage: ChatMessage = {
        ...
        timestamp:
          context.currentAssistantMessage?.timestamp ||
          options.timestamp ||
          Date.now(),
      };
  ...
  private processAssistantMessage(
    ...
    // Extract message ID for tracking streaming messages
    ...
    if (message.message?.content && Array.isArray(message.message.content)) {
      for (const item of message.message.content) {
        if (item.type === "text") {
          if (options.isStreaming) {
            this.handleAssistantText(item, context, options, messageId);
          } else {
