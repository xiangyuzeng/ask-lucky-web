...
export function useStreamParser() {
  ...
  // Track the current streaming message ID to avoid stale closure issues
  // This ref persists across renders and provides immediate access to the current ID
  const currentMessageIdRef = useRef<string | null>(null);

  // Track accumulated content to avoid stale closure issues with currentAssistantMessage
  ...
  const adaptContext = useCallback(
    (context: StreamingContext): ProcessingContext => {
      return {
        ...
        // Current message ID for tracking streaming messages
        currentMessageIdRef,

        // Accumulated content for tracking streamed text
    ...
    (line: string, context: StreamingContext) => {
      try {
        ...
        if (data.type === "claude_json" && data.data) {
          ...
          // Reset tracking on abort
          currentMessageIdRef.current = null;
          accumulatedContentRef.current = "";
        } else if (data.type === "done") {
          // Reset tracking when response completes
          currentMessageIdRef.current = null;
          accumulatedContentRef.current = "";
        }
      } catch (parseError) {
